    ...
    def load_data(self):
        ...
        for line in lines:
            ...
            data.append({
                ...
                'cloth_mask': os.path.join(self.args.data_root_path, 'cloth-mask', cloth_img),
            })
        return data
...
class DressCodeTestDataset(InferenceDataset):
    def load_data(self):
        ...
        for sub_folder in ['upper_body', 'lower_body', 'dresses']:
            ...
            pair_txt = os.path.join(self.args.data_root_path, sub_folder, 'test_pairs_paired.txt' if self.args.eval_pair else 'test_pairs.txt')
            ...
            for line in lines:
                ...
                data.append({
                    ...
                    'cloth_name': os.path.join(sub_folder, cloth_img),
                    'mask': os.path.join(self.args.data_root_path, sub_folder, 'agnostic_masks', person_img.replace('.jpg', '.png')),
                    'cloth_mask': os.path.join(self.args.data_root_path, sub_folder, 'cloth-mask', cloth_img),
                })
        return data
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            "The path to the base model to use for Wevaluation. This can be a local path or a model identifier from the Model Hub."
        ),
    )
    parser.add_argument(
        "--resume_path",
...
def repaint(person, mask, result):
    _, h = result.size
    # kernal_size = h // 50                      # BOBA: control kernel, orig: 50
    # if kernal_size % 2 == 0:
    #     kernal_size += 1
    ...
    if mask_np.ndim == 2:                           # for dresscode mask
...
def main():
    ...
    for batch in tqdm(dataloader):
        ...
        for i in range(len(person_results)):
            ...
            person_output_path = os.path.join(args.output_dir, "person", person_name)
            ...
            if not os.path.exists(os.path.dirname(cloth_output_path)):

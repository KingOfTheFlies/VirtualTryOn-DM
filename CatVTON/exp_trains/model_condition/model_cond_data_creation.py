...
class InferenceDataset(Dataset):
    def __init__(self, args):
        ...
        self.data = self.load_data(args.sub_folder)
...
class VITONHDTestDataset(InferenceDataset):
    def load_data(self, sub_folder):
        assert os.path.exists(pair_txt:=os.path.join(self.args.data_root_path, sub_folder + '_pairs.txt')), f"File {pair_txt} does not exist."
        with open(pair_txt, 'r') as f:
            lines = f.readlines()
        self.args.data_root_path = os.path.join(self.args.data_root_path, sub_folder)
        output_dir = os.path.join(self.args.output_dir, "vitonhd", sub_folder, 'unpaired' if not self.args.eval_pair else 'paired')
        data = []
        for line in lines:
...
def parse_args():
    ...
    parser.add_argument(
        ...
        # default="zhengchong/CatVTON",
        help=(
            "True if attn from pth, False if from huggingface (resume_path == zhengchong/CatVTON)"
        ),
    )
    ...
    parser.add_argument(
        "--cloth_eval_result",
        action="store_true",
        help="Whether or not to save the all conditions separatly.",
    )
...
def main():
    ...
    pipeline = CatVTONPipeline(
        ...
        load_pth_attn=args.load_pth_attn,
    ...
    args.output_dir = os.path.join(args.output_dir, args.sub_folder)
    ...
    for batch in tqdm(dataloader):
        ...
        masks = (masks > 0.5).to(torch.uint8) * 255
        masks = to_pil_image(masks)

        
        person_results = results["person"]
        cloth_results = results["cloth"]

        for i in range(len(person_results)):
            result = person_results[i]
            person_name = batch['person_name'][i]
            cloth_name = batch['cloth_name'][i]
            person_output_path = os.path.join(args.output_dir, "person_condition", cloth_name)  # BOBA: making cloth name to result
            if not os.path.exists(os.path.dirname(person_output_path)):
                os.makedirs(os.path.dirname(person_output_path))
            mask_output_path = os.path.join(args.output_dir, "condition_mask", cloth_name)        # BOBA: making cloth name to result
            if not os.path.exists(os.path.dirname(mask_output_path)):
                os.makedirs(os.path.dirname(mask_output_path))
            cloth_output_path = os.path.join(args.output_dir, "cloth", cloth_name)
            if not os.path.exists(os.path.dirname(cloth_output_path)) and args.cloth_eval_result:
                os.makedirs(os.path.dirname(cloth_output_path))
            if args.repaint:
            ...
            if args.concat_eval_results:            # конкатит вход и выход (для показательных примеров)
                w, h = result.size
            ...
            if args.cloth_eval_result:
                cloth_results[i].save(cloth_output_path)

            
            shutil.copy(batch['mask_path'][i], mask_output_path)
            result.save(person_output_path)

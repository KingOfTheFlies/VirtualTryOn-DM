...
class CatVTONTrainerWUncondLoss:
    def __init__(
        self, 
        base_ckpt,
        train_dataset,
    ...
    def save_opt_checkpoint(self, path):
        ...
        print(f"Optimizer checkpoint saved to: {opt_path}")
    ...
    def train(self):
        ...
        while step < self.max_train_steps:
            ...
            mask_latent_concat = torch.cat([mask_latents, torch.zeros_like(mask_latents)], dim=self.concat_dim)
            ...
            uncond_image_pred_latents = noise_pred.split(noise_pred.shape[self.concat_dim] // 2, dim=self.concat_dim)[0]          #BOBA: splitting pred latents
            uncond_noise = noise.split(noise.shape[self.concat_dim] // 2, dim=self.concat_dim)[0]                                 #BOBA: splitting noise
            
            loss = F.mse_loss(uncond_image_pred_latents, uncond_noise)                                                                   # BOBA: loss with only orig image
            # print("LOSS:", loss)

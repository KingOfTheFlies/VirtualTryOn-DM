...
class CatVTONTrainer:
    ...
    def save_ft_attn_checkpoint(self, path):                    # TODO: split opt state and attn
        os.makedirs(os.path.dirname(path), exist_ok=True)
        att_ckp_path = path+".pth"
        torch.save(self.attn_modules.state_dict(), att_ckp_path)
        print(f"Attention module checkpoint saved to: {path}.pth")      # TODO: to logs

        opt_path = path+'_opt_state.pth'
        torch.save(self.optimizer.state_dict(), opt_path)
        print(f"Attention module checkpoint saved to: {path}.pth")

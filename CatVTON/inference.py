...
def main():
    ...
    for batch in tqdm(dataloader):
        ...
        masked_persons_results = results["masked_person"]
        masked_persons_results = to_pil_image(masked_persons_results)
        for i in range(len(person_results)):
            ...
            if args.concat_eval_results:            # конкатит вход и выход (для показательных примеров)
                ...
                concated_result.paste(masked_persons_results[i], (w*2, 0))
                concated_result.paste(cloth_images[i], (w, 0))  
                concated_result.paste(result, (w*3, 0))

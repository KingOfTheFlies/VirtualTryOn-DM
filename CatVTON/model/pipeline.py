    ...
    def __call__(
        ...
        **kwargs
        ...
        if cloth_mask is not None:
            cloth_mask = prepare_mask_image(cloth_mask).to(self.device, dtype=self.weight_dtype)
        # Mask image
        ...
        if cloth_mask is not None:
            cloth_mask_latent = torch.nn.functional.interpolate(cloth_mask, size=condition_latent.shape[-2:], mode="nearest")

        del image, mask, condition_image
        # Concatenate latents
        masked_latent_concat = torch.cat([masked_latent, condition_latent], dim=concat_dim)
        
        # BOBA: cloth mask condition
        if cloth_mask is not None:
            # print("CLOTH MASK", cloth_mask_latent.shape)
            mask_latent_concat = torch.cat([mask_latent, cloth_mask_latent], dim=concat_dim)
        else:
        ...
        latents_person, latents_cloth = latents.split(latents.shape[concat_dim] // 2, dim=concat_dim)
        
        image_person = 1 / self.vae.config.scaling_factor * latents_person
        image_person = self.vae.decode(image_person.to(self.device, dtype=self.weight_dtype)).sample
        image_person = (image_person / 2 + 0.5).clamp(0, 1)
        image_person = image_person.cpu().permute(0, 2, 3, 1).float().numpy()
        image_person = numpy_to_pil(image_person)

        # Декодируем вторую половину (cloth)
        image_cloth = 1 / self.vae.config.scaling_factor * latents_cloth
